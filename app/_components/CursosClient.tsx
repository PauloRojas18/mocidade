'use client'
// LOCALIZAÇÃO: app/_components/CursosClient.tsx
...
type Props = { cursos: Curso[]; jovens: Jovem[] }

export default function CursosClient({ cursos: cursosInit, jovens }: Props) {
  ...
  const salvarCurso = async () => {
    ...
    if (res.ok) {
      // Recarrega cursos via fetch para pegar o novo
      const atualizados = await fetch('/api/cursos').then(r => r.json())
      if (Array.isArray(atualizados)) setCursos(atualizados)
      fecharModal()
    }
    setSalvando(false)
  }
