'use client'
// LOCALIZAÇÃO: app/_components/FrequenciaClient.tsx
// Recebe dados iniciais como props — sem useEffect de fetch inicial, sem spinner
...
type Props = {
  aulasInit:    Aula[]
  praticasInit: Pratica[]
  jovensInit:   Jovem[]
  chamadaInit:  ChamadaItem[]
  cursosInit:   Curso[]
  hoje:         string
}
...
export default function FrequenciaClient({ aulasInit, praticasInit, jovensInit, chamadaInit, cursosInit, hoje }: Props) {
  const [aba,         setAba]         = useState<Tab>('cursos')
  const [view,        setView]        = useState<View>('lista')
  const [todosJovens, setTodosJovens] = useState<Jovem[]>(jovensInit)
  const [cursos,      setCursos]      = useState<Curso[]>(cursosInit)

  // ── Cursos ─────────────────────────────────────────────────────────────
  const [aulas,       setAulas]       = useState<Aula[]>(aulasInit)
  ...
  // ── Práticas ───────────────────────────────────────────────────────────
  const [praticas,       setPraticas]       = useState<Pratica[]>(praticasInit)
  const [membrosPratica, setMembrosPratica] = useState<Record<number, Jovem[]>>(() => {
    const mp: Record<number, Jovem[]> = {}
    praticasInit.forEach(p => {
      mp[p.id] = p.pratica_membros?.map(m => m.jovens).filter((j): j is Jovem => j !== null) ?? []
    })
    return mp
  })
  ...
  const [chamadaDia,  setChamadaDia]  = useState(() =>
    jovensInit.map(j => ({
      id:       j.id,
      nome:     j.nome,
      presente: chamadaInit.find(c => c.jovem_id === j.id)?.presente ?? false,
    }))
  )
  ...
  // Recarrega apenas aulas (pós criação de aula)
  ...
  const criarAula = async () => {
    ...
    if (res.ok) await recarregarAulas()
                  ...
                  <div className="px-5 py-4 flex flex-col gap-4 overflow-y-auto" style={{ maxHeight: '65vh' }}>
                    <div>
                      ...
                      {cursos.length === 0 ? (
                        ...
                        <CustomSelect value={novoCursoId} onChange={v => setNovoCursoId(v === '' ? '' : Number(v))} placeholder="Selecione um curso..."
