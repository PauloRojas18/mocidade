'use client'
// LOCALIZAÇÃO: app/_components/PraticasClient.tsx
...
export default function PraticasClient({ praticas }: { praticas: Pratica[] }) {
  return (
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-5">
        {praticas.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {[...praticas]
              .sort((a, b) => (b.pratica_membros?.length ?? 0) - (a.pratica_membros?.length ?? 0))
              .map((p, i) => {
                ...
                return (
                  <div key={p.id} className="bg-white rounded-xl overflow-hidden" style={{ border: `0.5px solid ${color.border}` }}>
                    <div className="p-4" style={{ background: color.header }}>
                      ...
                      {p.descricao && (
                        <p className="text-xs mt-2 leading-relaxed" style={{ color: 'rgba(255,255,255,0.75)' }}>{p.descricao}</p>
                      )}
                    </div>
                    <div className="p-3.5">
                      <div className="mb-3">
